...
class studentPersonalInfo: public checkLoginStudent
{
	public:
		studentPersonalInfo(string name, string pass):checkLoginStudent(name, pass)
		{}
...
//Class to check student Attendence
...
//Class to check display attendence all the students
...
class teacherAttendence:public checkLoginTeacher
{
	...
	public:
		...
		void deleteAttendence()
		{
			...
			cout<<"Try Again! The Date you have entered does not exist.";
		}
...
class teacherMarks:public checkLoginTeacher
{
	...
	public:
		...
		void addNewMarks()
		{
			...
			try{
				fstream file;
				fileName="C:\\Users\\Hp\\Downloads\\FAST\\3rd semester\\OOP\\MINIFLEX\\Teacher\\"+convert(username)+"\\Marks.txt";
				file.open(fileName.c_str(),ios::in);
				if (file.is_open())
				{
					while(!file.eof())
	...
	void deleteMarks()
		{
			...
			string ask;
			int index;
			cout<<"Enter the thing to delete: ";
			cin>>ask;
			for(int i=0; i<1; i++)
			{
				for (int j=0; j<days; j++)
...
//Class that shows teacher weekly plan
...
{	
				...
				if (result)
				{
					while(ch=='s' || ch=='S')
					{
						...
						cin>>ask;
							...
							{
								cout<<endl;
								studentPersonalInfo I1(name, pass);
								I1.printInfo();		
							}
							else if(ask==2)
							{
								//MARKS CLASS TO DO HERE 
								cout<<"MARKS"<<endl;
								studentMarks M1(name, pass);
								M1.displayMarks();
							}
							else if(ask==3)
							{
				...
				cin>>ask;
					...
					else if(ask==2)
					{
						...
						while(ask1!='0')
						{
							...
							{
								int totalLec;
								cout<<"Enter the total number of columns inserted: ";
								cin>>totalLec;
	...
	cout<<"Enter S/T to continue else Press Q to quit: ";
	cin>>ch;
	}
	cout<<"----------------------THE END----------------------"<<endl;
